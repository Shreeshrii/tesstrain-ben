...
def generate_font_image(ctx, font, exposure, char_spacing):
    ...
    with open(ctx.training_text, "r", encoding='utf-8') as gtText:
        ...
        for line in lines:
            ...
            # Info message to delete all related files if box file is of size zero
            if os.stat(str(gtoutputbase) + ".box").st_size == 0:
               log.info(f"DELETE files with box file of size zero {(str(gtoutputbase) + '.box')}")
               #os.remove(str(gtoutputbase) + ".*") 
               #Comment as this causes exception and end of processing because of missing files.
        ...
        with tqdm(
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                #except Exception as exc:
                    #log.info("Failed while generating images " + str(exc))
                #else:
                    pbar.update(1)
